...
import { calculateDeal, maxOfferByCapRate, maxOfferByCoC } from '@/lib/dealCalculator'
...
// --- Styles ---
const inputClass = 'w-full bg-void border border-green/20 rounded px-3 py-2.5 text-sm text-chrome focus:border-green/50 outline-none'
...
export function DealAnalyzer() {
  ...
  const { inputs, address, areaData, targetCapRate, targetCoC, savedDeals } = store
  ...
  const results = useMemo(() => {
    ...
    return calculateDeal(inputs, effectiveRate, stressTest)
  }, [inputs, effectiveRate, stressTest])

  const maxCapPrice = useMemo(() => {
    if (!results || targetCapRate <= 0) return null
    return maxOfferByCapRate(results.noi, targetCapRate / 100)
  }, [results, targetCapRate])

  const maxCoCPrice = useMemo(() => {
    if (!results || targetCoC <= 0) return null
    return maxOfferByCoC(inputs, effectiveRate, targetCoC / 100)
  }, [inputs, effectiveRate, targetCoC, results])
  ...
  async function handleLookup() {
    ...
    try {
      ...
      if (data.geo.stateAbbr) {
        store.updateInputs({ location: data.geo.stateAbbr })
      }
    } catch {
  ...
  const numChange = (field: keyof typeof inputs) => (e: React.ChangeEvent<HTMLInputElement>) => {
    const val = e.target.value === '' ? 0 : parseFloat(e.target.value)
    store.updateInputs({ [field]: isNaN(val) ? 0 : val })
  }
  ...
  return (
    <SectionCard title="Deal Analyzer" accent="green">
      ...
      <div className="grid grid-cols-2 gap-x-3 gap-y-2.5">
        <Field label="Units">
          <input type="number" min={1} value={inputs.units || ''} onChange={numChange('units')} className={inputClass} inputMode="numeric" />
        </Field>
        ...
        <Field label="Down %">
          <input type="number" min={0} max={100} value={inputs.downPaymentPercent || ''} onChange={numChange('downPaymentPercent')} className={inputClass} inputMode="numeric" />
        </Field>
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-x-3 gap-y-3">
            <Stat
              ...
              sub={
                <GlowBadge variant={results.dealQuality === 'Strong' ? 'bullish' : results.dealQuality === 'Decent' ? 'neutral' : 'bearish'}>
                  {results.dealQuality}
                </GlowBadge>
              }
            />
          ...
          <div className="space-y-2.5">
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                className="w-14 bg-void border border-green/20 rounded px-2 py-1.5 text-xs text-chrome text-center focus:border-green/50 outline-none"
                inputMode="decimal"
              />
          ...
          <div className="space-y-2">
            {savedDeals.map((deal) => (
              ...
              >
                ...
                >
                  <div className="text-xs text-chrome truncate">{deal.address}</div>
